...
Page({
  data: {
    ...
    cardId: '0', // 使用的次卡ID
    deliveryTimeOptions: [
      { name: '正常24小时(免费)', value: 24, fee: 0 },
      { name: '加急12小时(+6元加急)', value: 12, fee: 6 }
    ],
    selectedDeliveryTime: 24, // 默认24小时
    deliveryFee: 0, // 默认免费
    expectedReturnTime: '', // 预计送回时间
  ...
  async doneShow() {
    ...
    this.setData({
      ...
      curAddressData: mockData.curAddressData,
      scoreDeductionRules: mockData.scoreDeductionRules,
      balance: mockData.balance,
      userScore: mockData.userScore,
      peisongType: this.data.peisongType,
      isNeedLogistics: 1,
      allGoodsAndYunPrice: 397.00, // 商品总价
      yunPrice: 10.00, // 运费
      amountLogistics2: 5.00, // 附加运费
      totalScoreToPay: 100, // 可用积分
      deductionMoney: 10.00, // 积分抵扣金额
      couponAmount: 30.00 // 优惠券金额
    })
    
    // 不再调用这些方法,因为已经设置了静态数据
  ...
  // 计算送回时间范围的辅助函数
  calculateReturnTimeRange(pickupTimeStr, hours) {
    const pickupTime = new Date(pickupTimeStr)
    const returnStartTime = new Date(pickupTime.getTime() + hours * 60 * 60 * 1000)
    const returnEndTime = new Date(returnStartTime.getTime() + 60 * 60 * 1000) // 加1小时
    ...
    const startTimeFormat = formatTime(returnStartTime)
    const endTimeFormat = formatTime(returnEndTime)

    return `${startTimeFormat.fullDate} ${startTimeFormat.time}-${endTimeFormat.time}前`
  },
  // 选择时间段
  onTimeSelect(e) {
    ...
    if (this.data.selectedDay === '明天') {
      targetDate.setDate(targetDate.getDate() + 1)
    } else if (this.data.selectedDay === '后天') {
      targetDate.setDate(targetDate.getDate() + 2)
    }
    
    // 设置选中的上门时间
    const pickupDate = new Date(targetDate.setHours(parseInt(hours), parseInt(minutes), 0))
    const pickupDateStr = pickupDate.format('yyyy-MM-dd hh:mm:ss')
    
    // 计算预计送回时间范围
    const expectedReturnTime = this.calculateReturnTimeRange(pickupDateStr, this.data.selectedDeliveryTime)
    
    this.setData({
      selectedTime: timeSlot,
      selectedTimeSlot: timeSlot,
      dateStart: pickupDateStr,
  ...
  handleDeliveryTimeChange(e) {
    ...
    const selected = e.detail.value === '加急12小时(+6元加急)' ? 12 : 24
    const fee = selected === 12 ? 6 : 0
    
    // 计算预计送回时间范围
    const expectedReturnTime = this.calculateReturnTimeRange(this.data.dateStart, selected)
    
    this.setData({
      selectedDeliveryTime: selected,
      deliveryFee: fee,
      expectedReturnTime: expectedReturnTime
    })

    this.processYunfei() // 重新计算费用
  },
})